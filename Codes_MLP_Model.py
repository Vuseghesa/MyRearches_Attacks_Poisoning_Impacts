...
def modify_data(df, percentage):
    ...
    # Modify temperature values randomly within a normal range
...
def train_and_evaluate(df, modification_type, percentage):
    selected_columns = ["day", "month", "time", "temp", "location_encoded","anomaly"]
    ...
    X = data_selected[["day", "month", "time", "temp_normalized", "location_encoded"]].values
    y = data_selected["anomaly"].values

    # Train-Test Split
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)

    # Build MLP Model (3 Layers)
